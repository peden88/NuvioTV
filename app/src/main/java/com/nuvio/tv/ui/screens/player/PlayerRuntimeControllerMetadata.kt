...
/**
 * Prefetches the next episode through the fork's existing StreamPrefetchCache
 * and PrefetchSelectionSupplier, but uses the same outro/post-play timing rule
 * as auto-next. The one-minute projection gives addon discovery, selection,
 * debrid pre-resolution and CDN prewarm time to finish without the old
 * six-minute double-scrape cycle.
 */
internal fun PlayerRuntimeController.maybePrefetchNextEpisodeForBinge(
    ...
    if (!shouldPrefetchNextEpisodeStreams(
            ...
            isLive = _playbackTimeline.value.isLive,
